...
struct PC {
	...
	string Output(int number) {
		return "PC " + to_string(number) + ":\n"
			+ "ID: " + to_string(id)
			+ "\ndate of production: " + date
			+ "\nwarranty: " + warranty
			+ "\nmodel: " + model
			+ "\nhard drive: " + hard_drive
			+ "\nRAM: " + to_string(ram).substr(0, to_string(ram).find(".") + 3) + " GB"
			+ "\nprice: " + to_string(price).substr(0, to_string(price).find(".") + 3) + " leva \n\n";
	}
};
...
void Output() {
	system("cls");
	for (int i = 0; i < number_pcs; i++) cout << pcs[i].Output(i + 1);
	system("pause");
}
...
void Save() {
	ofstream file;
	file.open("pcs.txt");
	for (int i = 0; i < number_pcs; i++) file << pcs[i].Output(i+1);
	file.close();
	cout << "Created list of PCs saved to pcs.txt.";
}
